...
function rellenarCausasExpulsion(){
	crearAjax();
	objAjax.open("GET","http://localhost/Trabajo Cliente/php/listarCausasExpulsion.php", true);
	objAjax.send();
...
function crearCausaExpulsion(nuevaCausaExp){
	if(nuevaCausaExp===""){
		...
		crearAjax();
		objAjax.open("GET","http://localhost/Trabajo Cliente/php/crearCausaExpulsion.php?causa="+nuevaCausaExp, true);
		objAjax.send();
